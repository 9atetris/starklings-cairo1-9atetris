...
fn main() {
    ...
    let mut my_sec_initial =
        'X'; // Finish this line like the example! What's your favorite short string?
    // Try a letter, try a number, try a special character, try a short string!
    if is_alphabetic(ref my_sec_initial) {
        ('Alphabetical!').print();
    } else if is_numeric(ref my_sec_initial) {
        ('Numerical!').print();
    } else {
        ('Neither alphabetic nor numeric!').print();
    }
